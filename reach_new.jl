...
function affine_map(A,b,C,d)
	if rank(C) == length(d)
		return A*inv(C), b + A*inv(C)*d, true  # TODO(yue)
	end
	...
	return Aₒ[good_idxs,:], bₒ[good_idxs], false  # TODO(yue)
end
...
# 	ap2output   = Dict{Vector{BitVector}, Tuple{Matrix{Float64},Vector{Float64}} }() # Dict from ap -> (A′,b′) ouput constraints
	ap2output   = Dict{Vector{BitVector}, Tuple{Matrix{Float64},Vector{Float64}, Bool} }() # Dict from ap -> (A′,b′) ouput constraints
